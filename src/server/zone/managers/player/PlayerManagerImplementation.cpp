...
int PlayerManagerImplementation::awardExperience(CreatureObject* player, const String& xpType,
	...
	int xp;
	if (amount <= 0 || xpType.contains("force") ||
	xpType.contains("fs_")||
	xpType.contains("saber"))
	{		xp = playerObject->addExperience(xpType, (int) amount * localMultiplier * globalExpMultiplier);
	} else if (xpType == "combat_general" ||
	xpType.contains("jedi")){
		xp = playerObject->addExperience(xpType, (int) (amount * localMultiplier * globalExpMultiplier * 5));
	} else {
