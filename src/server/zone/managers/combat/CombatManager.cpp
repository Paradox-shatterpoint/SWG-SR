...
float CombatManager::getDefenderToughnessModifier(CreatureObject* defender, int attackType, int damType, float damage) {
	...
	armor = getArmorObject(defender, damType);
...
int CombatManager::getHitChance(TangibleObject* attacker, CreatureObject* targetCreature, WeaponObject* weapon, int damage, int accuracyBonus) {
	...
	if (damage > 0) {
		...
		/*if (def == "saber_block") {
			if (!attacker->isTurret() && (weapon->getAttackType() == SharedWeaponObjectTemplate::RANGEDATTACK) && ((System::random(100 + (playerAccTotal / 5))) < targetCreature->getSkillMod(def)))
				return RICOCHET;
			else return HIT;
		}*/
		...
		if (def == "saber_block") {
			if (attacker->isPlayerCreature()) {
				if (!attacker->isTurret() && (weapon->getAttackType() == SharedWeaponObjectTemplate::RANGEDATTACK) && ((System::random(100 + (accTotal / 8))) < targetCreature->getSkillMod(def)))
					return RICOCHET;
				else return HIT;
			} else {
