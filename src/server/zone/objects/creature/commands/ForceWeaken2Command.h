	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (res == SUCCESS) {
			...
			if (creatureTarget != NULL) {
				...
				buff->setAttributeModifier(CreatureAttribute::HEALTH, float(creatureTarget->getHAM(CreatureAttribute::HEALTH) * -0.2));
				buff->setAttributeModifier(CreatureAttribute::ACTION, float(creatureTarget->getHAM(CreatureAttribute::ACTION) * -0.2));
				buff->setAttributeModifier(CreatureAttribute::MIND, float(creatureTarget->getHAM(CreatureAttribute::MIND) * -0.2));
